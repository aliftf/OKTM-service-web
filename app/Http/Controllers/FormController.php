...
class FormController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $path_ksm = 'public/file/ksm/' . pathinfo($ksmfile->getClientOriginalName(), PATHINFO_FILENAME) . '-' . auth()->user()->mahasiswa->nim . '.' . $ksmfile->extension();
        ...
        if ( $tipe ==  "Pengajuan Penggantian KTM"){
            ...
            $path_surat_kehilangan = 'public/file/surat-kehilangan/' . pathinfo($surat_kehilanganFile->getClientOriginalName(), PATHINFO_FILENAME) . '-' . auth()->user()->mahasiswa->nim . '.' . $surat_kehilanganFile->extension();
        ...
        if ( $tipe !=  "Pengajuan Penggantian KTM"){
            ...
            $path_ktm = 'public/file/ktm/' . pathinfo($ktmFile->getClientOriginalName(), PATHINFO_FILENAME) . '-' . auth()->user()->mahasiswa->nim . '.' . $ktmFile->extension();
            Storage::put($path_ktm, file_get_contents($ktmFile));
        }
        
        $bukti_pembayaranFile = $request->file('bukti_pembayaran');
        $path_bukti_pembayaran = 'public/file/bukti-pembayaran/' . pathinfo($bukti_pembayaranFile->getClientOriginalName(), PATHINFO_FILENAME) . '-' . auth()->user()->mahasiswa->nim . '.' . $bukti_pembayaranFile->extension();
